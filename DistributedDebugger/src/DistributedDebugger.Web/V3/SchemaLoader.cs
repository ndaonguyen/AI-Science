    ...
    private static IReadOnlyList<SchemaDoc> LoadFrom(string dir)
    {
        if (!Directory.Exists(dir))
        {
            Console.Error.WriteLine(
                $"[v3/schemas] folder not found at {dir} — analyses will run without schema context. " +
        ...
        foreach (var file in files)
        {
            try
            {
                ...
                Console.Error.WriteLine($"[v3/schemas] loaded {name} ({content.Length:N0} chars)");
            }
            catch (Exception ex)
            {
                // One bad file shouldn't take down the rest. Log and skip.
                Console.Error.WriteLine($"[v3/schemas] failed to load {file}: {ex.Message}");
            }
        }
        return docs;
    }
