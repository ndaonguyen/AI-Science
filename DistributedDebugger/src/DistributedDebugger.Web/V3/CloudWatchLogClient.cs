...
public sealed class CloudWatchLogClient : IDisposable
{
        ...
        CancellationToken ct)
    {
        ...
        Console.Error.WriteLine(
            $"[v3/cw] search → logGroup={logGroup} " +
    ...
    private AmazonCloudWatchLogsClient GetClient(string region, string environment)
    {
        ...
        {
            Console.Error.WriteLine(
                $"[v3/cw] GetClient: building new client region={region} profile={profile}");
            ...
            if (credentials is null)
            {
                ...
                Console.Error.WriteLine(
                    $"[v3/cw] WARNING: credential load returned null for profile '{profile}'. " +
                    ...
                    "actual AWS call runs. Check the [v3/cw] message above for the cause.");
                return new AmazonCloudWatchLogsClient(endpoint);
            }
            Console.Error.WriteLine(
                $"[v3/cw] credentials loaded: {credentials.GetType().Name}");
    ...
    private static SessionAWSCredentials? LoadCredentialsViaCli(string profileName)
    {
        try
        {
            ...
            {
                Console.Error.WriteLine(
                    "[v3/cw] could not start 'aws' — is the AWS CLI installed and on PATH? " +
            ...
            {
                Console.Error.WriteLine(
                    $"[v3/cw] aws CLI exited {proc.ExitCode} for profile '{profileName}': " +
                    $"{stderr.Trim()}\n" +
            ...
            {
                Console.Error.WriteLine(
                    $"[v3/cw] aws CLI returned empty stdout for profile '{profileName}'");
                return null;
            }
        ...
        catch (Exception ex)
        {
            ...
            Console.Error.WriteLine(
                $"[v3/cw] credential load threw {ex.GetType().Name}: {ex.Message}");
            return null;
        }
    }
